...
function bindEvents() {
  ...
  els.manualLocationBtn.addEventListener('click', toggleManualCoordinates);
...
function ensureInitialParcel() {
  if (!state.parcels.length) {
    ...
    state.parcels.push({ id, farmName: '', name: 'Ma parcelle', variety: 'generic', latitude: '', longitude: '', altitude: '', stageBDate: '', baseTemp: 5, observations: [] });
...
function loadParcelIntoForm() {
  ...
  els.manualCoordinates.classList.add('hidden');
  updateLocationSummary(p);
  renderPhenology();
}
...
function saveParcelFromForm() {
  ...
  Object.assign(p, {
    farmName: els.farmName.value.trim(),
...
function newParcel() {
  const id = crypto.randomUUID ? crypto.randomUUID() : String(Date.now());
  state.parcels.push({ id, farmName: '', name: `Parcelle ${state.parcels.length + 1}`, variety: 'generic', latitude: '', longitude: '', altitude: '', stageBDate: '', baseTemp: 5, observations: [] });
  state.activeParcelId = id;
  persistState();
  renderParcelSelect();
  loadParcelIntoForm();
...
function toggleManualCoordinates() {
  const willShow = els.manualCoordinates.classList.contains('hidden');
  els.manualCoordinates.classList.toggle('hidden', !willShow);
  els.manualLocationBtn.textContent = willShow ? 'Masquer les coordonnées' : 'Saisir des coordonnées';
  if (willShow) els.latitude.focus();
}

function updateLocationSummary(parcel = activeParcel()) {
  if (!parcel || parcel.latitude === '' || parcel.longitude === '') {
    els.locationSummary.textContent = 'Aucune position enregistrée';
    els.locationStatus.textContent = '';
    return;
  }
  const lat = Number(parcel.latitude).toLocaleString('fr-FR', { minimumFractionDigits: 4, maximumFractionDigits: 4 });
  const lon = Number(parcel.longitude).toLocaleString('fr-FR', { minimumFractionDigits: 4, maximumFractionDigits: 4 });
  const altitude = Number.isFinite(Number(parcel.altitude)) && parcel.altitude !== '' ? Math.round(Number(parcel.altitude)) : '—';
  els.locationSummary.textContent = `Latitude ${lat} · longitude ${lon} · altitude ${altitude} m`;
  els.locationStatus.textContent = 'Position trouvée';
}
...
function useGPS() {
  ...
  navigator.geolocation.getCurrentPosition(pos => {
    ...
    const p = activeParcel();
    p.latitude = Number(els.latitude.value);
    p.longitude = Number(els.longitude.value);
    if (Number.isFinite(pos.coords.altitude)) p.altitude = Math.round(pos.coords.altitude);
    persistState();
    updateLocationSummary(p);
    els.gpsBtn.disabled = false;
    els.manualCoordinates.classList.add('hidden');
    els.manualLocationBtn.textContent = 'Saisir des coordonnées';
...
async function refreshWeather(force) {
  ...
  try {
    ...
    if (Number.isFinite(Number(forecast.elevation))) {
      p.altitude = Math.round(Number(forecast.elevation));
      persistState();
      updateLocationSummary(p);
    }
...
function renderPhenology() {
  ...
  const fullSeries = computeSeries(activeWeather, startDate, base, p.observations || []);
  const stopped = stopSeriesAtTerminalStage(fullSeries, p);
  const series = stopped.series;
  ...
  const cycleFinished = Boolean(terminal && terminal.date <= todayIso);
  const next = cycleFinished ? null : STAGES.find(s => s.gdd > current.adjustedGdd);
  ...
  els.nextDate.textContent = next
    ? (predicted ? formatDate(predicted.date) : 'Au-delà de la période prévisionnelle')
    : (cycleFinished ? `Dernier stade atteint le ${formatDate(terminal.date)}` : '—');
  ...
  renderAlerts(series, current, currentStage, next, predicted, cycleFinished);
  renderForecastTable(series, todayIso, cycleFinished, terminal);
...
function terminalStageInfo(series, parcel) {
  const terminalStage = STAGES[STAGES.length - 1];
  const observedDate = (parcel.observations || [])
    .filter(observation => observation.stage === terminalStage.id)
    .map(observation => observation.date)
    .filter(Boolean)
    .sort((a, b) => a.localeCompare(b))[0] || null;
  const estimatedDay = series.find(day => day.adjustedGdd >= terminalStage.gdd) || null;

  if (observedDate && estimatedDay) {
    return observedDate <= estimatedDay.date
      ? { stage: terminalStage, date: observedDate, source: 'Observée' }
      : { stage: terminalStage, date: estimatedDay.date, source: 'Estimée' };
  }
  if (observedDate) return { stage: terminalStage, date: observedDate, source: 'Observée' };
  if (estimatedDay) return { stage: terminalStage, date: estimatedDay.date, source: 'Estimée' };
  return null;
}

function stopSeriesAtTerminalStage(series, parcel) {
  const terminal = terminalStageInfo(series, parcel);
  return terminal
    ? { series: series.filter(day => day.date <= terminal.date), terminal }
    : { series, terminal: null };
}
...
function renderStageHistory(series = null, todayIso = isoDate(new Date())) {
  ...
  if (!series) {
    ...
    const startDate = p.stageBDate || `${new Date().getFullYear()}-03-01`;
    const fullSeries = computeSeries(activeWeather, startDate, base, p.observations || []);
    series = stopSeriesAtTerminalStage(fullSeries, p).series;
  }
...
function renderGddChart(series = null, todayIso = isoDate(new Date())) {
  ...
  const terminal = terminalStageInfo(series, p);
  const current = [...series].reverse().find(day => day.date <= todayIso) || series[0];
  const currentStage = stageForGdd(current.adjustedGdd);
  const cycleFinished = Boolean(terminal && terminal.date <= todayIso);
  const nextStage = cycleFinished ? null : STAGES.find(stage => stage.gdd > current.adjustedGdd);
  const lastForecast = [...series].reverse().find(day => day.type === 'forecast') || series[series.length - 1];

  els.chartCurrentGdd.textContent = `${Math.round(current.adjustedGdd)} °C·j`;
  els.chartNextThreshold.textContent = nextStage ? `${nextStage.id} · ${nextStage.gdd} °C·j` : 'Cycle suivi terminé';
  els.chartForecastEnd.textContent = cycleFinished ? 'Cumul arrêté' : `${Math.round(lastForecast.adjustedGdd)} °C·j`;
  els.gddChartStatus.textContent = cycleFinished
    ? `Cumul arrêté au dernier stade le ${formatDate(terminal.date)}.`
    : `Du ${formatDate(series[0].date)} au ${formatDate(series[series.length - 1].date)} · stade actuel ${currentStage.id}.`;
...
function renderAlerts(series, current, currentStage, next, predicted, cycleFinished = false) {
  const alerts = [];
  const today = isoDate(new Date());
  if (!cycleFinished && predicted) {
  ...
  const sensitive = !cycleFinished && ['D','E','E2','F','F2','G','H','I'].includes(currentStage.id);
...
function renderForecastTable(series, today, cycleFinished = false, terminal = null) {
  if (cycleFinished) {
    els.forecastBody.innerHTML = '';
    els.forecastTableWrap.classList.add('hidden');
    els.weatherStatus.textContent = `Dernier stade atteint le ${formatDate(terminal.date)} : les prévisions phénologiques sont arrêtées.`;
    return;
  }
