...
const STATION = Object.freeze({ name: 'Marsillargues', latitude: 43.6343, longitude: 4.1706, altitude: 2 });
...
function ensureInitialParcel() {
  if (!state.parcels.length) {
    ...
    state.parcels.push({ id, farmName: '', name: 'Ma parcelle', variety: 'generic', stageBDate: '', baseTemp: 5, observations: [] });
...
function newParcel() {
  const id = crypto.randomUUID ? crypto.randomUUID() : String(Date.now());
  state.parcels.push({ id, farmName: '', name: `Parcelle ${state.parcels.length + 1}`, variety: 'generic', stageBDate: '', baseTemp: 5, observations: [] });
  state.activeParcelId = id;
  persistState();
  renderParcelSelect();
  loadParcelIntoForm();
...
async function refreshWeather(force) {
  ...
  try {
    ...
    const historicalUrl = `https://archive-api.open-meteo.com/v1/archive?latitude=${STATION.latitude}&longitude=${STATION.longitude}&start_date=${start}&end_date=${historyEnd}&daily=temperature_2m_max,temperature_2m_min&timezone=Europe%2FParis`;
    const forecastUrl = `https://api.open-meteo.com/v1/forecast?latitude=${STATION.latitude}&longitude=${STATION.longitude}&daily=temperature_2m_max,temperature_2m_min&timezone=Europe%2FParis&forecast_days=16`;
...
function renderPhenology() {
  ...
  if (!p || !activeWeather) {
    ...
    els.stageBbch.textContent = 'Chargement météo nécessaire';
...
function renderStageHistory(series = null, todayIso = isoDate(new Date())) {
  const p = activeParcel();
  if (!p || !activeWeather) {
    els.stageHistoryStatus.textContent = 'Chargez les données météo pour calculer les dates.';
...
function renderGddChart(series = null, todayIso = isoDate(new Date())) {
  ...
  if (!p || !activeWeather) {
    els.gddChartStatus.textContent = 'Chargez les données météo pour afficher la courbe.';
